...
<body>
	<div class="topbar">
		<a class="return" onclick="history.go(-1)"></a> 房产冻结确认
	</div>
	<div class="showbox">
		<form class="cmxform form-horizontal" id="godownForma" name="godownForma" action="${ctx}/app/completeTask" method="post">
			<div class="submit_input">
				<input id="taskId" name="taskId" type="hidden" value="${wfTask.wfTaskInfo.id }"/>
              	...
              	<input name="fp_isHouseFrozen" id="fp_isHouseFrozen" value="1" type="hidden" />
				<p class="selectRadio ed" name="fp_isHouseFrozen1"
					id="fp_isHouseFrozen1" selected="selected"
					onclick="toggleRadio(this);">房产是否冻结*</p>

				<p class="last">
					<input type="text" name="fp_houseFrozenTime" id="fp_houseFrozenTime" placeholder="请选择冻结时间" />
				</p>
			</div>
			...
			<div class="submit_box">
				<p class="submit">
					<input type="button" value="提  交"
						onclick="$('input[name=fp_appStatus]').val('pass');$('input[name=fp_status]').val('1');$('input[name=fp_approvalStep]').val('9');$('input[name=fp_approvalStatus]').val('0');javascript:submitAudit('godownForma');">
				</p>
			</div>
		</form>
	</div>
    ...
    function toggleRadio(obj) {
		if ($(obj).hasClass("ed")) {
			$("#fp_isHouseFrozen").val("0");
			$(obj).removeClass("ed");
			$(obj).removeAttr("selected");
		} else {
			$("#fp_isHouseFrozen").val("1");
			$(obj).addClass("ed");
      ...
      function submitAudit(domId){
    	  var fp_isHouseFrozen = $("#fp_isHouseFrozen").val();
    	  if(fp_isHouseFrozen == null || fp_isHouseFrozen == '' || fp_isHouseFrozen == '0'){
    		  alert("请勾选房产是否冻结   ");
    		  return false;
    	  }
	...
	<script type="text/javascript">
	    $(function () {
	        ...
	        $("#fp_houseFrozenTime").mobiscroll($.extend(opt['date'], opt['default']));
