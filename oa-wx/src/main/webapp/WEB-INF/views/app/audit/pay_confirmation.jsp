  ...
  <body>
   ...
   担保费用收取确认
    	...
    	<form class="cmxform form-horizontal" id="godownForma" name="godownForma" action="${ctx}/app/completeTask" method="post">
    		<div class="submit_input">
              <input id="taskId" name="taskId" type="hidden" value="${wfTask.wfTaskInfo.id }"/>
              	...
              	<input name="fp_isGuaranteeFee" id="fp_isGuaranteeFee" value="1" type="hidden" />
				<p class="selectRadio ed" name="fp_isGuaranteeFee1" id="fp_isGuaranteeFee1" selected="selected" onclick="toggleRadio(this);">担保费用是否已收*</p>
				
              	<p class="last"><input type="text" name="fp_guaranteeFeeTime" id="fp_guaranteeFeeTime" placeholder="请选择收取日期"/></p>
			</div>
			...
			<div class="submit_box">
				<p class="submit">
					<input type="button" value="提  交"
						onclick="$('input[name=fp_appStatus]').val('pass');$('input[name=fp_status]').val('1');$('input[name=fp_approvalStep]').val('14');$('input[name=fp_approvalStatus]').val('0');javascript:submitAudit('godownForma');">
				</p>
			</div>
         </form>
    </div>
    ...
    function toggleRadio(obj) {
		if ($(obj).hasClass("ed")) {
			$("#fp_isGuaranteeFee").val("0");
			$(obj).removeClass("ed");
			$(obj).removeAttr("selected");
		} else {
			$("#fp_isGuaranteeFee").val("1");
			$(obj).addClass("ed");
      ...
      function submitAudit(domId){
    	  var fp_isGuaranteeFee = $("#fp_isGuaranteeFee").val();
    	  if(fp_isGuaranteeFee == null || fp_isGuaranteeFee == '' || fp_isGuaranteeFee == '0'){
    		  alert("请勾选担保费用是否已收  ");
    		  return false;
    	  }
    ...
    <script type="text/javascript">
	    $(function () {
	        ...
	        $("#fp_guaranteeFeeTime").mobiscroll($.extend(opt['date'], opt['default']));
