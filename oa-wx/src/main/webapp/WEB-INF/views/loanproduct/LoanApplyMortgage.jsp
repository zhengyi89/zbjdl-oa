...
<script type="text/javascript"
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/static/default/css/mine.css" />
</head>
<body>
	<script>
	...
	<form method="get" id="godownForma" name="godownForma"
		...
		<input type="hidden" id="type" name="type" value="${type}"> <input
			type="hidden" id="merchant" name="merchant" value="${merchant}">
			...
			<div class="weui_cell weui_cell_select weui_select_after">
				...
				<div class="weui_cell_bd weui_cell_primary">
					<select class="weui_select" name="provinceCode" id="provinceCode" onchange="getCity(this.value);"
						...
						<%-- 
						<option selected="selected" value=""></option>
						<c:forEach var="caseDto" items="${cityList}">  
	                        <option value="${caseDto.cityCode}">  
	                            <c:out value="${caseDto.cityName}" />  
	                    </c:forEach> 
	                    --%> 
					</select>
				</div>
		...
		<div class="weui_cells weui_cells_form">
			<div class="weui_cell">
				...
				<div id="payAmount_div_reg" class="loan_pop_warn" style="display: none;">只能输入数字<i class="outer"></i><em class="inner"></em></div>
		...
		<div class="weui_cells_title" style="font-size: 16">抵押房产信息</div>
		<div class="weui_cells weui_cells_form">
			<div class="weui_cell weui_cell_select weui_select_after">
				<div class="weui_cell_hd">
					<label for="" class="weui_label">房产类型</label>
				</div>
				<div class="weui_cell_bd weui_cell_primary">
					<select class="weui_select" name="houseType" id="houseType">
						<option selected="selected" value=""></option>
						<option value="0">住宅</option>
						<option value="1">商用房</option>
						<option value="2">自建房</option>
					</select>
				</div>
			</div>
			<div class="weui_cell">
				<div class="weui_cell_hd">
					<label class="weui_label">楼盘地址</label>
				</div>
				<div class="weui_cell_bd weui_cell_primary">
					<input class="weui_input" type="text" maxlength="60"
						name="houseAddress" id="houseAddress" placeholder="请输入抵押房产楼盘地址" />
				</div>
			</div>
			<div class="weui_cell">
				<div class="weui_cell_hd">
					<label class="weui_label" style="font-size: 14">房屋面积(㎡)</label>
				</div>
				<div class="weui_cell_bd weui_cell_primary">
					<input class="weui_input" type="text" name="houseArea" onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')"
						maxlength="10" id="houseArea" placeholder="请输入抵押房产房屋面积" />
				</div>
			</div>
		</div>
