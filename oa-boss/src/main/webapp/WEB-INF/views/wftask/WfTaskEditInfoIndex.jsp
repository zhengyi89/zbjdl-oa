      ...
      <div class="content_main">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <form class="cmxform form-horizontal" action="${ctx}/wftask/wfTaskEditInfoIndex" method="get" id="godownForma"
				  name="godownForma">
        ...
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <div class="col-xs-12">
            <q:table queryService="queryService" queryKey="queryTaskEditInfo" formId="godownForma"
				...
				<q:column title="业务编号" value="${taskno}" width="10%"/> 
				<q:column title="经办人姓名" value="${username}" width="10%"/>
				<q:column title="中介姓名" value="${mediationname}" width="12%"/>
				<q:column title="卖方姓名" value="${sellername}" width="10%"/>
				<q:column title="买方姓名" value="${buyername}" width="10%"/>
				<q:column title="业务来源" escapeHtml="false" value="${_textResource.getSysText('BUSINESSSOURCE', businesssource)}" width="12%"/>
				<q:column title="解押金额" value="${_messageFormater.formatNumber(sellersolutionamount)}" width="12%"/>
				<q:column title="创建时间" width="14%" >
					<fmt:formatDate value="${createtime}" pattern="yyyy-MM-dd HH:mm:ss" />
				</q:column>
				<q:column title="操作" escapeHtml="false" width="6%">
					<c:if test="${status == '0'}">
						<a class="margr4" href="${ctx}/wftask/toWfTaskInfoEdit?id=${id}&back=edit">编辑</a>
					</c:if>
				</q:column>
			</q:table>				
            </div>
